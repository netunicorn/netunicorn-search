...
class PipelineExecutor:
    def __init__(self, executor_id: str = None, gateway_endpoint: str = None):
        ...
        self.gateway_endpoint: str = (
            gateway_endpoint or os.environ["PLACEHOLDER_GATEWAY_ENDPOINT"]
        )
        if self.gateway_endpoint[-1] == "/":
            self.gateway_endpoint = self.gateway_endpoint[:-1]

        self.executor_id: str = (
            executor_id or os.environ.get("PLACEHOLDER_EXECUTOR_ID") or "Unknown"
        )
    ...
    def request_pipeline(self) -> None:
        ...
        pipeline_filename = f"<placeholder>.pipeline"
...
def main():
    executor = PipelineExecutor(
        executor_id="7caa212d-37f8-4e7d-b878-807ce792e5ad",
        gateway_endpoint="redacted",
    )
    executor()
...
# Results:
# Total memory consumption of the executor: 3.2 MB + Python interpreter memory
# where:
#     pipeline binary size: 19 KB
#     HttpClient and requests data: 2.4 Mb
#     ProcessPool and results of pipeline execution: 0.7 Mb
